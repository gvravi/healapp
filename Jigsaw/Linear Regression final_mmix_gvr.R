...
colSums(is.na(mmix))
sapply(mmix, function(x) sum(is.na(x)))
summary(mmix)
...
#Replacing Missing Values for all columns with each column mean(own)
for(i in 1:ncol(mmix)){
  mmix[is.na(mmix[,i]),i] <- mean(mmix[,i], na.rm = TRUE)
}

sapply(mmix, function(x){
  x[is.na(x)]
